...
const { readFile, writeFile, spinnerStart } = require('@arcane-cli/utils');
...
const DEFAULT_CLI_HOME = '.arcane-cli';
...
class Git {
  ...
  async prepare() {
    // check 缓存主目录
    this.checkHomePath();
    await this.checkGitServer(); // check 仓库类型
  ...
  async getRemoteBranchList(type) {
    ...
    if (type === VERSION_RELEASE) {
      reg = /.+?refs\/tags\/release\/(\d+\.\d+\.\d+)/g; // 1f4a5s6fjidasjfi refs/tags/release/1.0.0
    } else {
      reg = /.+?refs\/heads\/dev\/(\d+\.\d+\.\d+)/g; // 2d4f6j7a4kfka refs/heads/dev/1.0.1
    }
  ...
  async initAndAddRemote() {
    log.info('执行git初始化');
    await this.git.init(false, this.dir);
